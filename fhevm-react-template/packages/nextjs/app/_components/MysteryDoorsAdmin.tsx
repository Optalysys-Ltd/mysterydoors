  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6 text-gray-900">
      ...
      <div className="text-center mb-8 text-black">
        ...
        <p className="text-gray-600">Only the contract owner can interact with these functions.</p>
        <p className="text-gray-600">Click Collate Leaderboard to collate the latest results to be decrypted</p>
      </div>


      {/* Leaderboard */}
      <div className="grid grid-cols-1text-black overflow-y-auto overflow-x-auto">
        ...
        <table className="table table-zebra">
          ...
          <tbody>
            {Object.entries(mysteryDoors.collatedLeaderboard).map(([playerName, correctGuesses]) => {
              return (<tr id={playerName} key={playerName}>
                <th>{playerName}</th>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-black">
        <button
          className={mysteryDoors.isPlayersCorrectGuessesDecrypted ? successButtonClass : primaryButtonClass}
          disabled={!mysteryDoors.canDecryptPlayersCorrectGuesses || mysteryDoors.isProcessing}
          onClick={mysteryDoors.decryptPlayersCorrectGuessesHandles}
        >
          {mysteryDoors.canDecryptPlayersCorrectGuesses
            ? "🔓 Decrypt leaderboard scores"
            : mysteryDoors.isPlayersCorrectGuessesDecrypted
              ? `✅ Decrypted length: ${mysteryDoors.clearPlayersCorrectGuesses?.length || 0}`
              : mysteryDoors.isDecryptingPlayersCorrectGuesses
                ? "⏳ Decrypting..."
                : "❌ No leaderboard to decrypt"}
        </button>

        <button
          className={secondaryButtonClass}
          disabled={!mysteryDoors.canUpdate || mysteryDoors.isDecryptingPlayersCorrectGuesses || mysteryDoors.isGameEnded}
          onClick={mysteryDoors.adminCallEndGame}
        >
          {mysteryDoors.isGameEnded
            ? "Game already ended"
        ...
        <button
          className={secondaryButtonClass}
          disabled={!mysteryDoors.canUpdate || mysteryDoors.isDecryptingPlayersCorrectGuesses}
          onClick={mysteryDoors.adminCallCollateLeaderboard}
        >
          {mysteryDoors.canUpdate
            ? "Collate leaderboard"
            : mysteryDoors.isProcessing
              ? "⏳ Processing..."
              : "❌ Cannot collate leaderboard"}
        </button>
      ...
      {/* Game grid */}
      {mysteryDoors.isOccupiedPositionsDecrypted && mysteryDoors.clearOccupiedPositions !== undefined &&
        <MysteryDoorsGridMeasured bgUrl={"/mysterydoors3.png"} isSelected={isSelected} isDoorOccupied={isOccupied} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={sectionClass}>
          ...
          <div className="space-y-3">
            ...
            {printProperty("Decrypting", mysteryDoors.isDecryptingPlayersCorrectGuesses)}
            {printProperty("Processing", mysteryDoors.isProcessing)}
            {printProperty("Can Get Leaderboard", mysteryDoors.canGetPlayersCorrectGuesses)}
            {printProperty("Can Decrypt Leaderboard", mysteryDoors.canDecryptPlayersCorrectGuesses)}
